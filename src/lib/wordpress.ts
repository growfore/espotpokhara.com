...
const API_BASE = "https://tieupone.com.np/wp-json/wp/v2"

export async function getPosts(): Promise<WPPost[]> {
  const res = await fetch(`${API_BASE}/posts?per_page=20&_embed=wp:featuredmedia`, {
...
export async function getPostBySlug(slug: string): Promise<WPPost | null> {
  const res = await fetch(`${API_BASE}/posts?slug=${slug}&_embed=wp:featuredmedia`, {
    next: { revalidate: 3600 },
  })
